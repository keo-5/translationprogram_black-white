...
def generate_translation_and_vocab(english_text, api_key, major_info, user_level):
    try:
        ...
        if user_level == "고등학교 1~2학년 (기초/내신)":
            level_instruction = (
                "- 번역 수준: 수능 및 내신 기초를 다지는 고등학생 눈높이입니다. 구조가 복잡한 문장은 끊어 읽기 형태로 직관적으로 번역해 주고, 문장 구조가 한눈에 보이게 다듬어 주세요.\n"
                "- 단어장 수준: 핵심 '수능 필수 영단어 및 숙어'를 5개 이상 뽑아주시고, 단어의 뜻과 예문을 포함해 주세요."
            )
        elif user_level == "고등학교 3학년 (수능/심화입시)":
            level_instruction = (
                "- 번역 수준: EBS 연계 교재 고난도 지문 분석 수준입니다. 은유적이거나 꼬여있는 문장을 '주제와 핵심 요지'가 명확하게 전달되도록 해설지처럼 깔끔하게 의역해 주세요.\n"
                "- 단어장 수준: 모의고사 1등급 고난도 어휘 위주로 발췌하고, 문맥적 해석을 돕는 '구문 독해 팁'을 추가해 주세요."
            )
        elif user_level == "대학 신입생 / 입문자 (학부 1~2학년)":
            level_instruction = (
                "- 번역 수준: 전공 원서를 처음 읽는 저학년 수준입니다. 생소한 학술 전문 용어가 등장하면 단어 옆에 괄호()를 치고 '일상적인 비유나 쉬운 예시 설명'을 본문 속에 직접 주입해 주세요.\n"
                "- 단어장 수준: 전공 기초 어휘 위주로 뽑고, 기초 개념 지식을 친절하게 풀어서 기술해 주세요."
            )
        elif user_level == "학부 고학년 (학부 3~4학년)":
            level_instruction = (
                "- 번역 수준: 전공 서적 교재나 학술 논문 번역본 표준 규격에 맞추어, 실제 세미나에서 사용하는 매끄럽고 세련된 학술적 어조로 의역해 주세요.\n"
                "- 단어장 수준: 학술 전문 용어 위주로 발췌하고, 졸업 연구에 인용 서술할 수 있는 정확한 전공 학술적 정의를 기재해 주세요."
            )
        elif user_level == "대학원생 / 연구원 (석박사 단계)":
            level_instruction = (
                "- 번역 수준: 실무 및 연구 전문가용입니다. 기초적인 개념 설명은 배제하고, 해외 유명 저널의 초록(Abstract)처럼 극도로 정교하고 조밀하며 압축된 프로페셔널한 번역 톤을 유지해 주세요.\n"
                "- 단어장 수준: 최신 연구 트렌드와 직결된 고난도 핵심 키워드 위주로 간결하게 주석을 달아주세요."
            )

        system_prompt = (
            f"너는 학생들이 영어 지문, 전공 서적, 논문을 마스터하도록 돕는 대한민국 최고의 AI 맞춤형 교육 멘토야. "
            f"현재 문서의 학문 분야는 **[{major_info}]**이며, 독자의 학업 레벨은 **[{user_level}]** 단계야.\n\n"
            f"아래 제시된 학년별 가이드라인을 100% 준수하여 두 가지 결과물을 완벽하게 작성해줘:\n\n"
            f"{level_instruction}\n\n"
            f"1. [자연스러운 수준별 의역]\n"
            f"위 지침에 맞춰 문체와 부가설명 깊이를 정밀 조절한 한국어 해석본을 작성해줘.\n\n"
            f"2. [필수 개인 맞춤형 단어장]\n"
...
if UPSTAGE_API_KEY == "UPSTAGE_API_KEY" or not UPSTAGE_API_KEY:
    ...
    with col_major:
        major_category = st.selectbox(
            "문서의 학문 분야를 지정하세요:",
            ["컴퓨터공학 / IT / AI", "의학 / 보건 / 바이오", "경영학 / 경제학", "자연과학 / 일반공학", "법학 / 행정학", "인문학 / 사회과학", "일반 수능 외국어 영역", "기타 (직접 입력)"]
        )
    with col_level:
